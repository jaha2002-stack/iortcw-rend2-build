#!/usr/bin/env python3
"""
Kit E: linear-distance cubemap shadows v1
...
static void RB_Mode3RenderDlightShadowCubemapDepthPass( drawSurf_t *drawSurfs, int numDrawSurfs )
{
    ...
    for ( lightIndex = 0; lightIndex < backEnd.refdef.num_dlights && lightIndex < maxLights; lightIndex++ )
    {
        ...
        for ( face = 0; face < faces; face++ )
        {
            ...
            /* Kit E: write RGB-packed linear distance into the color cubemap. */
            FBO_AttachImage( tr.renderCubeFbo, tr.shadowCubemaps[lightIndex], GL_COLOR_ATTACHMENT0_EXT, face );
...
def patch_local_d(path):
    ...
    path.write_text(s, encoding="utf-8")
    print(f"patched Kit E uniform enum: {path}")
...
def patch_shade_d(path):
    ...
    if "UNIFORM_DLIGHTSHADOWMODE" not in s:
        ...
        if n == 0:
            raise SystemExit(f"Could not insert Kit E dlight shader uniforms in {path}")
    ...
    if s != original:
        path.write_text(s, encoding="utf-8")
    print(f"patched Kit E shade uniforms/binding: {path}")
...
def patch_dlight_shaders_d(vp_path, fp_path):
    ...
    shadowfill_fp = fp_path.with_name("shadowfill_fp.glsl")
    shadowfill_fp.write_text(SHADOWFILL_FP_E, encoding="utf-8")
    print(f"replaced Kit E dlight shader and shadowfill shader: {vp_path}, {fp_path}, {shadowfill_fp}")
...
print("Kit E shader-side depth compare diagnostic patch completed.")
